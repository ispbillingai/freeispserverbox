...
// Median of the samples that actually had contact. Returns -1 if none did.
int capture() {
  int s[6], n = 0;
  while (touchDown() && n < 6) {
    int v = posRead();
    if (v >= 30) s[n++] = v;                    // RULE 2
    delay(60);
  }
...
#define NBOX 16
#define BH   (320 / NBOX)           // 20px per box
const char *NAME = "ABCDEFGHIJKLMNOP";
int anchor[NBOX];                   // the reading measured for each box
...
void drawBoxes(int hot, const String &note) {
  ...
  for (int i = 0; i < NBOX; i++) {
    int y = i * BH;
    if (i == hot) tft.fillRect(0, y + 1, 480, BH - 2, C_ACC);
    tft.drawFastHLine(0, y, 480, C_EDGE);
    tft.setTextSize(2);
    tft.setTextColor(i == hot ? C_BG : C_TXT, i == hot ? C_ACC : C_BG);
    tft.setCursor(16, y + 3); tft.print(NAME[i]);
    if (anchor[i] > 0) {            // show what this box measured
      tft.setTextColor(i == hot ? C_BG : C_OK, i == hot ? C_ACC : C_BG);
      tft.setCursor(52, y + 3); tft.print(anchor[i]);
    }
  }
  tft.setTextSize(1); tft.setTextColor(C_LABEL, C_BG);
  tft.setCursor(200, 4); tft.print(note);
}
...
void setup() {
  ...
  for (int i = 0; i < NBOX; i++) anchor[i] = 0;
  tft.begin();
  tft.fillScreen(C_BG);
...
void loop() {
  if (!testing) {
    // ---- CALIBRATE: walk A..H, one box at a time -------------------------
    ...
    anchor[idx] = v;
    Serial.printf("box %c raw=%d\n", NAME[idx], v);
    idx++;
    if (idx < NBOX) {
      drawBoxes(idx, "good - now the MIDDLE of the blue box");
    } else {
      Serial.print("TABLE:");
      for (int i = 0; i < NBOX; i++) Serial.printf("  %c=%d", NAME[i], anchor[i]);
      Serial.println();
      // The gaps are the verdict. A gap smaller than the tap error means
      // those two boxes cannot be told apart, no matter how good the code.
      Serial.print("GAPS: ");
      int tight = 0;
      for (int i = 1; i < NBOX; i++) {
        int g = abs(anchor[i] - anchor[i-1]);
        Serial.printf("%c-%c=%d  ", NAME[i-1], NAME[i], g);
        if (g < 60) tight++;
      }
      Serial.printf("\n%d of %d gaps are under 60 counts (unreliable)\n",
                    tight, NBOX - 1);
      testing = true;
      drawBoxes(-1, "TEST: press any box - it should light up");
    }
    return;
  }
  ...
  if (!touchDown()) { delay(80); return; }
  int v = capture();
  if (v < 0) { Serial.println("test tap: no contact"); return; }
  int best = 0, bestd = 32767;
  for (int i = 0; i < NBOX; i++) {
    int d = abs(v - anchor[i]);
    if (d < bestd) { bestd = d; best = i; }
  }
  Serial.printf("TEST raw=%d -> box %c (anchor %d, off by %d)\n",
                v, NAME[best], anchor[best], bestd);
  drawBoxes(best, "raw " + String(v) + " -> box " + String(NAME[best]));
}
