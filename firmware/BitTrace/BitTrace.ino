...
// Per-bit sweep. 0x02 is the state BoxCal reached by accident and is the
// only bus state that ever produced a clean 16-row gradient on this glass.
const uint8_t PATTERN[8] = {0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x3F};
const char *PARKNAME[8]  = {"0x00 all-LOW  ", "0x01 D0       ", "0x02 D1 BoxCal",
                            "0x04 D2       ", "0x08 D3       ", "0x10 D4       ",
                            "0x20 D5       ", "0x3F all-HIGH "};
#define NPARK 8
...
static void park(int state) {
  ...
  uint8_t pat = PATTERN[state];
  for (int i = 0; i < 6; i++) {
    pinMode(PIN_D[i], OUTPUT);
    digitalWrite(PIN_D[i], ((pat >> i) & 1) ? HIGH : LOW);
  }
...
static void paintPhase() {
  ...
  for (int s = 0; s < NPARK; s++) {
    Serial.printf("baseline IMAGE=%s park=%s  z/y:", white ? "WHITE" : "DARK ", PARKNAME[s]);
    for (int i = 0; i < 2; i++) { int z, y; readPair(s, &z, &y); Serial.printf("  %4d/%4d", z, y); delay(60); }
    Serial.println();
  }
...
void setup() {
  Serial.begin(115200);
  delay(200);
  Serial.println("\n>>> BitTrace: fixed DARK image, one bus bit at a time, z/y per pattern");
  tft.begin();
  tft.fillScreen(C_BG);
...
void loop() {
  ...
  // dark image only: the image is no longer the variable under test
}
