...
- **Horn sound window:** the mouth stares at the front panel, so `FIR_BottomLid` and `FIR_CurvedLid` both carry the same five 80 x 4 mm slots at shell X = -113..-33, Z = 34..66 - clear of the switch pocket below (Z32), of the lid bolts at Z72, and of the counterbored bolt at X-132.  That is 1600 mm² against a 102 mm mouth of 8171 mm², so **about 20 % open**: the horn will be audibly quieter inside the box than in open air, and `FIR_ShellCheck` states that number every run rather than letting it be discovered after the print.
